...
0. 选择 `1` 制作模板，或 `2` 仅设置客户 VPS 备份位置（跳过模板、网桥和宿主机更新策略）。
...
6. 选择客户 VPS 备份存储，必须在线、启用且支持 `backup`；`0` 表示暂不指定。官网同步节点资源后匹配，官网手动设置优先，套餐权限与额度不变。
...
脚本校验官方镜像后，在独立镜像安装当前发行版官方更新和基础软件；所有镜像准备完成后才替换/创建模板，再验证制作结果。此流程不备份模板、不从旧备份执行 `qmrestore`。客户 VPS 备份目标在上述菜单配置为节点级偏好，不随模板克隆继承。
...
使用节点备注中的独立行 `PPFLIGHT_CUSTOMER_BACKUP_V1=<storage-id>`，`-` 表示未指定。保留其他备注，以 PVE 配置 digest 防止并发覆盖，保存后读取核对。新节点返回空配置、没有 digest 时，使用 PVE 原生配置锁，在锁内重新确认仍为空后初始化；非空配置缺失 digest 仍停止。不写定时备份任务，不调整备份权限或保留份数。保存目标在模板构建前完成，即使随后构建失败也保留该独立配置。

官网同步资源后检查存储的节点归属、在线状态和 backup 内容类型。首次匹配后如目标丢失或不可用，不改选其他存储。官网手动保存（包括不配置）以及历史明确默认存储优先于安装器；已有 VPS 的备份绑定保持不变。没有标记的旧节点保持原兼容逻辑。节点配置需要读取权限（Sys.Audit）；无法读取时页面提示重新检查。

这只是指定备份目的地，不代表已经完成真实备份及恢复验收。

`zfspool` 只支持虚拟磁盘和容器，不能直接存放 vzdump 备份文件。若需要在同一 ZFS 池中保存备份，应由管理员创建独立数据集并注册为支持 backup 的 Directory 存储；建议设置容量限额，避免备份耗尽客户磁盘池。该方式仍与虚拟磁盘共用故障域，不能代替异机备份。安装器不会自行创建数据集或改变存储类型。
