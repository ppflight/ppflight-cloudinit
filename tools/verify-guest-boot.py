...
def verify(image, vendor, timeout=360, firmware="seabios"):
    ...
    with tempfile.TemporaryDirectory(prefix='pf-boot-') as directory:
        ...
        args=['qemu-system-x86_64','-enable-kvm','-cpu','host','-m','2048','-smp','2',
              ...
              '-device','scsi-hd,drive=osdisk,bus=scsi0.0,bootindex=100',
              '-drive','file='+str(root/'seed.iso')+',if=none,id=seed,media=cdrom,readonly=on',
              '-device','ide-cd,drive=seed,bus=ide.1,unit=0,bootindex=101',
              '-boot','strict=on',
